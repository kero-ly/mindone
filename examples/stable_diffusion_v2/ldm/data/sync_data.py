...
import tarfile
import pandas as pd
from .laion_gen_csv_annot import gen_csv
...
def sync_data(from_path, to_path, start_sample_idx, end_sample_idx):
    """
    ...
    if get_device_id() % min(get_device_num(), 8) == 0 and not os.path.exists(sync_lock):
        ...
        for f in to_path:
            with tarfile.open(f, 'r') as archive:
                _logger.info("finish unzip" + f)
                archive.extractall(os.path.dirname(f))
            os.remove(f)
        _logger.info("===finish data unzip===")

        ### generate summary csv
        gen_csv("/cache", one_csv_per_part=True, folder_prefix="part", start_sample_idx=start_sample_idx, end_sample_idx=end_sample_idx, del_part_csvs=True)
        _logger.info("===finish generate summary csv, path: /cache/merged_imgp_text.csv ===")

        ### allocate csv to each device
        # pd.read_csv("/cache/merged_imgp_text.csv")
        # use shards
